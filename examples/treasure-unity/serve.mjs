...
if(!process.argv.includes('--practice'))bridge=await startLocalBridge({origins:[origin],port:companionPort,browserLogin:true});
...
const server=createServer(async(req,res)=>{
 try{
  ...
  let body,type;
  if (path === '/kit/browser.js' || path === '/kit/types.js') {
   body=await readFile(resolve(here,'../../dist',path.split('/').pop()));type='application/javascript';
  } else if(path==='/bootstrap.json'){
   ...
   body=Buffer.from(JSON.stringify({baseUrl:bridge?.baseUrl,loader:pick('.loader.js'),data:pick('.data'),framework:pick('.framework.js'),wasm:pick('.wasm')}));type='application/json';
  }else{
...
server.listen(port,'127.0.0.1',()=>{
 ...
 console.log('ゲームの「ChatGPTでログイン」から接続してください。');
});
